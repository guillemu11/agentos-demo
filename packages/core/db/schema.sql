...
-- ─── BAU Campaign Builder: build state with preview gate ──────────────────
CREATE TABLE IF NOT EXISTS bau_builds (
  ...
  -- Phase A output (mutable during preview gate)
  slot_map            JSONB,
  variants            JSONB,
  rows                JSONB,          -- Synthetic DE rows (with fake image IDs)
  images_base64       JSONB,          -- fakeId → data:image/png;base64,...
  ...
  -- State machine: building | preview-ready | refining | pushing | published | failed
...
CREATE INDEX IF NOT EXISTS idx_bau_builds_user_status
  ON bau_builds(user_id, status);
CREATE INDEX IF NOT EXISTS idx_bau_builds_created
  ON bau_builds(created_at DESC);
...
-- Competitor Intel reference scores (2026-04-20)
