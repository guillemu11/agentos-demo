...
const merkleIconSvg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100">
  ...
  <text x="32" y="74" font-family="Inter,Arial,Helvetica,sans-serif" font-size="62" font-weight="800" fill="#FFFFFF" letter-spacing="1">M</text>
</svg>`;
const merkleIcon = `data:image/svg+xml;base64,${Buffer.from(merkleIconSvg).toString('base64')}`;

const emiratesLogo = imgToBase64('../assets/Emirates-Emblem.jpg');
...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .cover-divider { width: 80px; height: 3px; background: var(--white); margin: 25px auto; }
  .cover-emirates-logo { width: 150px; margin: 20px auto; }
  ...
  .section-divider p { font-size: 14px; font-weight: 300; opacity: 1; color: var(--white); }
  .section-divider .divider-line { width: 60px; height: 3px; background: var(--white); margin: 20px auto; }
  ...
  .back-line { width: 60px; height: 2px; background: var(--white); margin: 30px auto; }
</style>
</head>
<body>
...
<div class="page back-cover">
  ...
  <div style="margin-top: 30px;">
    <img src="${emiratesLogo}" style="width: 100px;" alt="Emirates">
  </div>
</div>
...
(async () => {
  ...
  const outputPath = path.resolve(__dirname, 'AgentOS-Emirates-Client.pdf');
  await page.pdf({
    path: outputPath,
    format: 'A4',
  ...
  await browser.close();
  console.log(`Emirates Client PDF generated: ${outputPath}`);
})();
